...
TEST_CASE("ComputeEditDistance: perfect match has NM=0", "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(10, BAM_CMATCH)};
  auto const query = EncodeSeq("ACGTACGTAC");
  auto const target = EncodeSeq("ACGTACGTAC");
...
TEST_CASE("ComputeEditDistance: mismatches under M ops", "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(5, BAM_CMATCH)};
  auto const query = EncodeSeq("ATGTA");
  auto const target = EncodeSeq("AAGAA");
...
TEST_CASE("ComputeEditDistance: insertion adds to NM", "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(3, BAM_CMATCH), MakeCigar(2, BAM_CINS),
                                     MakeCigar(3, BAM_CMATCH)};
  auto const query = EncodeSeq("ACGTTACG");  // 8 bases
  auto const target = EncodeSeq("ACGACG");   // 6 bases
...
TEST_CASE("ComputeEditDistance: deletion adds to NM", "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(3, BAM_CMATCH), MakeCigar(2, BAM_CDEL),
                                     MakeCigar(3, BAM_CMATCH)};
  auto const query = EncodeSeq("ACGACG");     // 6 bases
  auto const target = EncodeSeq("ACGTTACG");  // 8 bases
...
TEST_CASE("ComputeEditDistance: soft clips excluded from NM", "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(3, BAM_CSOFT_CLIP), MakeCigar(5, BAM_CMATCH),
                                     MakeCigar(2, BAM_CSOFT_CLIP)};
  auto const query = EncodeSeq("NNNACGTANN");  // 10 bases (3S + 5M + 2S)
  auto const target = EncodeSeq("ACGTA");      // 5 bases (only matched region)
  CHECK(ComputeEditDistance(cigar, query, target) == 0);
}

TEST_CASE("ComputeEditDistance: mixed CIGAR with mismatches, ins, del",
          "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(4, BAM_CMATCH), MakeCigar(1, BAM_CINS),
                                     MakeCigar(2, BAM_CMATCH), MakeCigar(1, BAM_CDEL),
                                     MakeCigar(4, BAM_CMATCH)};
  auto const query = EncodeSeq("ACATCACTAAA");   // 11 bases
  auto const target = EncodeSeq("ACGTACGTAAA");  // 11 bases
...
TEST_CASE("ComputeEditDistance: =/X operators work correctly", "[lancet][hts][CigarUtils]") {
  ...
  std::vector<CigarUnit> const cigar{MakeCigar(3, BAM_CEQUAL), MakeCigar(1, BAM_CDIFF),
                                     MakeCigar(2, BAM_CEQUAL)};
  auto const query = EncodeSeq("ACGTAC");
  auto const target = EncodeSeq("ACGAAC");
...
TEST_CASE("CigarRefPosToQueryPos: soft clip advances query before match",
          "[lancet][hts][CigarUtils]") {
