...
class Edge {
 public:
  ...
  [[nodiscard]] auto IsSelfMirror() const noexcept -> bool {
    // Any 2 nodes connected to each other have 2 unique edges connecting them. src --> dst and dst
    // --> src The only exception to this rule are self-loops with +- or -+ edge, where there is
    // only one edge.
    return IsSelfLoop() && (mEdgeKind == EdgeKind::PLUS_MINUS || mEdgeKind == EdgeKind::MINUS_PLUS);
  }

  [[nodiscard]] auto MirrorEdge() const noexcept -> Edge {
    return Edge({mDstId, mSrcId}, RevEdgeKind(mEdgeKind));
  }

  template <typename HashState>
  friend auto AbslHashValue(HashState hash_state, Edge const& edge) -> HashState {
    return HashState::combine(std::move(hash_state), edge.mSrcId, edge.mDstId,
                              static_cast<u64>(edge.mEdgeKind));
  }

  auto operator==(Edge const& rhs) const -> bool {
